...
int	count_until_endl(char *buf, char *line, int *line_len)
{
	while (*line_len - ft_strlen(line) < BUFFER_SIZE)
	{
		if (buf[*line_len - ft_strlen(line)] == '\n')
		{
			*line_len += 1;
			return (1);
		}
		if (buf[*line_len - ft_strlen(line)] == '\0')
		{
			*line_len += 1;
			return (2);
		}
...
char	*recursive_get_line(char *buf, char *line, int fd, int *line_len)
{
	int			endln_found;
	char		*new_line;
	char		*subbuf;
	...
	if (buf[0] == '\0')
		return (line);
	endln_found = count_until_endl(buf, line, line_len);
	subbuf = ft_substr(buf, 0, *line_len - ft_strlen(line));
